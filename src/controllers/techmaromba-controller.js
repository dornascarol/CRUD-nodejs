...
const addNew = async (request, response) => {
    try {
        const { name, description, urlProfile, urlImage} = request.body
...
const clear = async (request, response) => {
    try {
        ...
        response.status(200).json({ message: `O id ${id} foi deletado`})
...
module.exports = {findAll, addNew, update, clear}
